...
# each element of the list data will be a list of length 75 with entries ordered according to the index lookup in utilities.py. Chunks of the data list 100,000 records or shorter are stored in pickle files
# returns a dictionary where key 'removed' is the number of records removed due to improper formating 'pickles' is a list of paths to created pickle files
def readAndPreprocess(file, pickleLocation):
    pickles = []
    results = {}
    try:
        data = []
        ...
        for i, row in enumerate(csv_f):
            ...
            if (len(row) == 75 and i != 0):  # i=0 is the headings
                ...
                row[31] = u.checkInt(row[31],0,99999)  # zipcode
                row[32] = u.checkNotBlank(row[32])  # vendorcountrycode
                row[33] = u.checkNotBlank(row[33])  # vendor_state_code
                row[34] = u.checkInt(row[34],0,100)  # vendor_cd
                row[35] = u.checkInt(row[35],0,100)  # congressionaldistrict
                row[36] = u.checkInt(row[36],0,9999999999)  # phoneno
                ...
                row[41] = u.checkInt(row[41],0,99999)  # placeofperformancezipcode
                ...
                row[50] = u.checkInt(row[50],0,500)  # numberofoffersreceived
                row[51] = u.checkNotBlank(row[51])  # organizationaltype
                row[52] = u.checkInt(row[52],0,2500000)  # numberofemployees
                row[53] = u.checkInt(row[53],0,500000000000)  # annualRevenue
                ...
                if (i % 100000 == 0 and i > 0):
                    path = pickleLocation + "save" + str(i / 100000) + ".p"
        ...
        path = pickleLocation + "savelast" + str(i / 100000) + ".p"
        pickle.dump(data, open(path, "wb"))
        pickles.append(path)

        results['removed'] = skippedRecords
        results['pickles'] = pickles
...
# deletes all of the pickle files in the pickles list
